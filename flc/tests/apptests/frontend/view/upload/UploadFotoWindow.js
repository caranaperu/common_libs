...
isc.WinUploadFoto.addProperties({
    ...
    _doUpload: function() {
        ...
        if (_file.files[0].type.match(/image.*/)) {
                ...
                if (request.readyState == 4) {
                    try {
                        ...
                        if (resp.response.success == "false") {
                            me.setStatus(resp.response.pe[0].msg);
                        } else if (resp.response.status == -1) {
                            me.setStatus(resp.response.data);
                        } else {
                            _file.value = '';
                            me.imgContainer.setSrc(resp.response.data.flc_file_uploaded);
            ...
            request.open('POST', glb_dataUrl + 'upload_photo_controller');
            request.send(data);
        }
    },
    initWidget: function() {
