 ...
 * @version 1.02
 * @since 1.00
 * @history 22-08-2021 , se agrego que a la hora de hacer un add o un update se pueda preguntar al usuario si desea hacerlo.
...
isc.DefaultController.addProperties({
    ...
    _saveRecord: function () {
        ...
        // Se requiere spolicit<r si se va a grabar , procedemos
        // a preguntar , de lo contrario ejecutamos normalmente.
        if (me._mantForm.askForSave === true) {
            isc.ask(me._mantForm.askMessage,
                function (value) {
                    if (value) {
                        me.__saveRecord(values,oldValues);
                    }
                });
        } else {
            me.__saveRecord(values,oldValues);
    ...
    /**
     *  Metodo de bajo nivel que efectua el update o agregar , ver el metodo _saveData para mayor explicacion
     *  detallada, basicamente se ha separado estea parte de la labor para permitir preguntar o no
     *  si se desea grabar , dada la naturaleza asincronica de isc.ask se requiere esta separacion.*
     *
     *  @see _saveData
     *  @private
     *  @param {array} values arreglo con la lista de valores a grabar
     *  @param {array} oldValues arreglo con la lista de valores previos en el caso de un update
     */
    __saveRecord: function (values,oldValues) {
        var me = this;
