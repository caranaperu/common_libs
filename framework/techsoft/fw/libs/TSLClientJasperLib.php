<?php
	...
	$jasperserver_url = "http://127.0.0.1:8080/jasperserver/services/";

	$SCHEDULING_WS_URI =  $jasperserver_url . "ReportScheduler?wsdl";
        $GLOBALS['webservices_uri'] = $jasperserver_url . "repository";

	$GLOBALS['namespace'] = "http://www.jaspersoft.com/namespaces/php";
	...
	function ws_runReport($user,$pass,$uri, $report_params, $output_params, &$attachments )
	{
		global $_SESSION;
		$max_execution_time = 120; // 2 mins.

		$connection_params = array("user" => $user, "pass" => $pass, "timeout" => $max_execution_time);
		$info = new SOAP_client($GLOBALS["webservices_uri"], false, false, $connection_params);
