...
class flcHeaderInputDataFetch extends flcInputData {
    public function process_input_data(array $p_input_data) {
        // simulated parser
        $this->operation = 'fetch';
        $this->model_fields = ['numero' => 100, 'descripcion' => '300'];
...
class flcHeaderInputDataAdd extends flcInputData {
    ...
    public function process_input_data(array $p_input_data) {
        // simulated parser
        $this->operation = 'add';
        $this->model_fields = ['numero' => 500, 'descripcion' => '300', 'customer_id' => 11];
    }
}
...
class flcHeaderInputDataUpdate extends flcInputData {
    ...
    public function process_input_data(array $p_input_data) {
        ...
        $this->model_fields = ['numero' => 500, 'descripcion' => 'ahora soy 600', 'customer_id' => 12];
    }
}
...
class factura_header_entity extends flcBaseEntity {
    public function __construct(flcDriver $p_driver, ?flcInputData $p_input_data) {
        ...
        // if input data exist
        if ($p_input_data) {
            if ($p_input_data->get_operation() == 'fetch' or $p_input_data->get_operation() == 'add'
                or $p_input_data->get_operation() == 'update' or $p_input_data->get_operation() == 'delete') {
                // get the model field values
                foreach ($p_input_data->get_model_fields() as $field => $value) {
                    $this->{$field} = $value;
                }
    ...
    public function &get_delete_constraints(?string $p_suboperation = null): ?flcConstraints {
        ...
        // where fields
        foreach ($this->input_data->get_filter_fields() as $field => $operator) {
            $wfields[] = [$field, $operator];
        }
        if (count($wfields) > 0) {
            $c = new flcConstraints();
            $c->set_where_fields($wfields);
        }
        return $c;
    }
    ...
    public function &get_fetch_constraints(?string $p_suboperation = null): ?flcConstraints {
        ...
        if ($p_suboperation == 'fetchjoined') {
            ...
            if ($this->input_data) {
                ...
                // sort fields
                $wfields = [];
                foreach ($this->input_data->get_sort_fields() as $field => $type) {
                    $wfields[] = [$field, $type];
                }
                if (count($wfields) > 0) {
                    if (!isset($c)) {
                    ...
                    $c->set_order_by_fields($wfields);
                }

                // pagination
                $start_row = $this->input_data->get_start_row();
                $end_row = $this->input_data->get_end_row();
