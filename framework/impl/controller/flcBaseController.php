 ...
 *
 */
namespace framework\impl\controller;
    ...
    public function index() {
        try {
            ...
            if ($this->is_logged_in()) {
                ...
                if ($this->execute_validations($operation, $this->model->get_all_fields('rc'))) {
                    ...
                    if (!$answer->is_success()) {
                        ...
                        if ($ex != null) {
                            ...
                            $this->output_data->set_answer_message('Error para parsear ['.$answer->get_return_code().']', $answer->get_return_code());
                        }
                    } else {
                ...
                // not logged in error
                if (!FLC::get_instance()->lang->load('flc_common')) {
                    throw new RuntimeException('Invalid language file flc_common (check deploy of library)- BC00008');
                }

                $this->output_data->add_process_error(0, FLC::get_instance()->lang->line('flc_common_not_logged_in'));
        ...
        if ($this->options && count($this->options)) {
            if (isset($this->options[$p_key])) {
                ...
                if ($p_key == 'input_data_processor') {
                    $base_class = 'framework\core\dto\flcInputDataProcessor';
                    $msg = 'input data processor';

                } elseif ($p_key == 'output_data') {
                    $base_class = 'framework\core\dto\flcOutputData';
                    $msg = 'output data';

                } elseif ($p_key == 'output_data_processor') {
                    $base_class = 'framework\core\dto\flcOutputDataProcessor';
                    $msg = 'output data processor';

                } else {
                    // model
                    $base_class = 'framework\core\entity\flcBaseEntity';
                    $msg = 'model';
                }
